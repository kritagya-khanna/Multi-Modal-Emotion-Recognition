...
def train_video_model(config_path):
    """
    Train video emotion recognition model
    ...
    X_train, X_val, X_test, y_train, y_val, y_test, label_mapping = load_and_prepare_video_data(config)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Train video emotion recognition model")
    parser.add_argument(
        "--config",
        type=str,
        default="config/video_config.json",
        help="Path to configuration file"
    )
    
    args = parser.parse_args()
    train_video_model(args.config)